...
const sb=window.supabase.createClient(cfg.SUPABASE_URL,cfg.SUPABASE_PUBLISHABLE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,storage:window.localStorage}});
const $=id=>document.getElementById(id);let products=[];let localCounts=JSON.parse(localStorage.getItem("calip_counts")||"[]");
...
async function logout(){await sb.auth.signOut();showOnly("loginView")}
